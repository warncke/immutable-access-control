...
function allowModel (args) {
    ...
    var deleted = states && states.includes('deleted')
    // get access rules
    ...
    if (states) {
        ...
        for (var i=0; i < statesLen; i++) {
            ...
            if (anyRules) {
                ...
                var allowAny = this.isRoleAllowed(allow, roles, anyRules, true)
            ...
            if (ownRules) {
                // pass changed flag so that undefined will be returned unless
                // allow state was changed by matching rule
                var allowOwn = this.isRoleAllowed(allow, roles, ownRules, true)
                // rule specifically allows access
                if (allowOwn === true) {
    ...
    if (anyRules) {
        ...
        var allowAny = this.isRoleAllowed(allow, roles, anyRules)
    ...
    if (ownRules && scope === 'own') {
        return this.isRoleAllowed(allow, roles, ownRules)
    }
    // otherwise return previous decision
    else {
        return allow
    }
}
    ...
    for (var i=0; i<segmentsLen; i++) {
        ...
        if (rules) {
            ...
            if (rules.method && rules.method[args.method]) {
                allow = this.isRoleAllowed(allow, roles, rules.method[args.method].allow)
            }
        }
 ...
 *
 * if the optional changed flag is set then undefined will be returned it no
 * rules were found that changed the allow state.
 *
 * @param {boolean} allow
 * @param {array} roles
 * @param {object} rules
 ...
 */
function isRoleAllowed (allow, roles, rules, changed) {
    ...
    for (var i=0; i < rolesLen; i++) {
        ...
        if (rules[roles[i]] === 1) {
            // return undefined if change flag set and no change otherise allow
            return changed && allow === true ? undefined : true
        }
    }
    // if there is a deny rule for all and there was no specific allow rule
    // then access is denied
    if (rules.all === 0) {
        // return undefined if change flag set and no change otherwise deny
        return changed && allow === false ? undefined : false
    }
    // return undefined if change flag set otherwise original allow state
    return changed ? undefined : allow
}
...
function requireValidArgsRoute (args) {
    ...
    assert.ok(typeof args.route === 'string' && typeof args.method === 'string', 'route and method required to request access for route')
